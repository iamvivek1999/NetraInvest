 ...
 *
 * This controller handled Razorpay order creation and payment verification.
 * It has been superseded by POST /api/v1/investments (on-chain via MetaMask).
 *
 * Retained for historical data integrity and legacy audit needs only.
 * Do NOT add new features here. Do NOT call from InvestModal.
 */
...
const blockchainLoggingService = require('../services/blockchainLogging.service');
const sendResponse = require('../utils/sendResponse');
const { ApiError } = require('../middleware/errorHandler');

/**
 * Creates a Razorpay Order and returns order details to the frontend
 * @route POST /api/v1/payments-legacy/create-order
 * @deprecated Use POST /api/v1/investments with on-chain txHash instead.
 */
const createOrder = async (req, res) => {
  console.warn(
    '[DEPRECATED] payment.controller.createOrder called. ' +
    'This Razorpay flow is deprecated. Use POST /api/v1/investments with on-chain txHash.'
  );
 ...
 * @route POST /api/v1/payments-legacy/verify
 * @deprecated Use POST /api/v1/investments with on-chain txHash instead.
 */
const verifyPayment = async (req, res) => {
  console.warn(
    '[DEPRECATED] payment.controller.verifyPayment called. ' +
    'This Razorpay flow is deprecated. Use POST /api/v1/investments with on-chain txHash.'
  );
